...
형식:
- `CUTE-<LAYER>-<DOMAIN>-<NNN>`

예시:
- `CUTE-WEB-NET-001`
- `CUTE-WEB-CORS-001`
- `CUTE-CORE-CONTROL-002`

구성:
- `LAYER`: WEB | CORE | SENSOR | DEPLOY
- `DOMAIN`: NET | CORS | STREAM | CONTROL | ENV | AUTH | DB
- `NNN`: 001부터 증가
...
```json
{
  "code": "CUTE-WEB-CORS-001",
  "severity": "error",
  "layer": "WEB",
  "domain": "CORS",
  "title": "CORS_ORIGINS 미설정",
  "userMessage": "백엔드 CORS 설정을 확인하세요.",
...
- `CUTE-WEB-NET-001`: API_BASE_URL 접근 실패
- `CUTE-WEB-CORS-001`: CORS 차단 가능성
- `CUTE-WEB-STREAM-001`: SSE 연결 실패
- `CUTE-WEB-CONTROL-001`: 제어 요청 실패
- `CUTE-CORE-CONTROL-001`: SENSOR_CONTROL_URL 미도달
- `CUTE-SENSOR-COLLECT-001`: collector 전송 실패
